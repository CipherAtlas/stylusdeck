...
struct EqBridgeResponse: Codable {
    let ok: Bool
    let connected: Bool
    let backend: String
    let detail: String
...
final class EqProcessor {
    ...
    func status(for band: String?) -> EqBridgeResponse {
        do {
            try ensureStarted()
            lock.lock()
    ...
    func setBand(_ band: String, value: Int) -> EqBridgeResponse {
        do {
            try ensureStarted()
            ...
            recalculateCoefficients()
            lock.unlock()
            return EqBridgeResponse(ok: true, connected: true, backend: "native-eq", detail: detail, band: band, value: clamped)
        } catch {
            return EqBridgeResponse(ok: false, connected: false, backend: "native-eq", detail: error.localizedDescription, band: band, value: value)
        }
    }

    func getOutputVolume() -> EqBridgeResponse {
        do {
            try ensureStarted()
            lock.lock()
            let value = Int((masterVolume * 100).rounded())
            lock.unlock()
            return EqBridgeResponse(ok: true, connected: true, backend: "native-eq", detail: detail, band: nil, value: value)
        } catch {
            return EqBridgeResponse(ok: false, connected: false, backend: "native-eq", detail: error.localizedDescription, band: nil, value: nil)
        }
    }

    func setOutputVolume(_ value: Int) -> EqBridgeResponse {
        do {
            try ensureStarted()
            let clamped = max(0, min(100, value))
            lock.lock()
            ...
            lock.unlock()
            return EqBridgeResponse(ok: true, connected: true, backend: "native-eq", detail: detail, band: nil, value: clamped)
        } catch {
            return EqBridgeResponse(ok: false, connected: false, backend: "native-eq", detail: error.localizedDescription, band: nil, value: value)
        }
    }
